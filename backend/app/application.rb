...
class Application
  ...
  def call(env)
    ...
    if req.path.match(/owners/) && req.post?
      ...
      data = JSON.parse(req.body.read) #code to unpackage the stringifiy JSON
      vehicleExists = Vehicle.find_by(vin: data["vin"]) #check if user xitst with that username? usernames must be uniq
      if vehicleExists
        return [200, { 'Content-Type' => 'application/json' }, [ {:error => "Vehicle with vin: #{vehicleExists.vin} already exists"}.to_json ]]   
      else
